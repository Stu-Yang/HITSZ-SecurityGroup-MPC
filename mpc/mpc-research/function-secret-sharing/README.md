...
**DS17**提出了为DPF函数秘密共享分布式密钥生成算法，但只适合较小输出群（原文只适用于{0,1}），**APR+22**利用了这一方式来提出了Gen^iDPF。**BCG+20**基于LPN假设为DPF密钥生成提出了分布式计算协议。**DGH+21**提出了函数秘密共享的分布式密钥生成算法（但未直接给出算法），相比于与Doerner和Shelat[DS17]的黑盒FSS密钥生成协议相比，其计算成本仅随域大小对数递增。BCG+21则在提出优化分布式点函数的同时，给出了安全分布式DCF密钥生成算法。**BCG+22**提出基于Expand-Accumulate Codes来生成相关随机性，文章提出了一种放松版本的DCF算法（RDCF），并为RDCF提出了分布式密钥生成算法。
